...
import {
  ...
  TRY_AGAIN_MESSAGE,
  WELCOME_MESSAGE,
} from 'src/components/NumberGuesser/NumberGuesser';
import * as useNumberGuesser from 'src/components/NumberGuesser/useNumberGuesser';
...
describe('test NumberGuesser', () => {
  describe('test initial state', () => {
    ...
    test('renders the secret number input', () => {
      ...
      expect(screen.getByRole('spinbutton')).toHaveAttribute('name', SECRET_NUMBER_INPUT_NAME);
    });
    ...
    test('renders the submit button', () => {
      ...
      expect(screen.getByRole('button').textContent).toEqual(SUBMIT_BUTTON_TEXT);
    });
  });
  ...
  describe('test result state', () => {
    test('renders the result message', async () => {
      ...
      expect(
        typeof parseInt(screen.getByText(RESULT_MESSAGE[2]).nextSibling?.textContent as string),
      ).toEqual('number');
  ...
  describe('test submit guess state', () => {
    ...
    test('renders the guess number input', async () => {
      ...
      expect(screen.getByRole('spinbutton')).toHaveAttribute('name', GUESS_NUMBER_INPUT_NAME);
    });
  ...
  describe('test try again state', () => {
    ...
    beforeEach(() => {
      jest.spyOn(useNumberGuesser, 'useNumberGuesser').mockReturnValue({
        ...renderHook(useNumberGuesser.useNumberGuesser).result.current,
        state: 'tryAgain',
        userGuesses,
      });
    });
    ...
    test('renders the tried numbers message', () => {
      ...
      expect(
        screen.getByText(`${TRIED_NUMBERS_MESSAGE} ${arrayToCommaSeparatedString(userGuesses)}.`),
      ).toBeInTheDocument();
    });
  ...
  describe('test correct guess state', () => {
    ...
    test('renders the play again button', () => {
      ...
      expect(screen.getByRole('button').textContent).toEqual(PLAY_AGAIN_BUTTON_TEXT);
    });
  });
});
